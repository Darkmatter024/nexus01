...
# S50 - ⭐ `.270` **DEVICE-VERIFIED** · SHIP `v1.14.271` = **SHIP B / INSTRUMENT** (2026-07-17)

## OWNER, IN CHAT: **".270 verified, ship b"**
`.270` SHIP A (cables sourced + `_rmConnHit` untangle) **PASSED ON DEVICE** — the fabricated cable layer is dead and confirmed gone on glass. Owner ordered **Ship B ahead of WET**, so **WET FINISH is now `.272`** (still queued; spec unchanged — `S44` items **1+2+3**, ⛔ **item 4 dropped forever**). **`.271` is now the one unverified ship in flight.**
⚠️ **WET has now been deferred THREE times** (`.269`→`.270`→`.271`→`.272`) and was **silently dropped once** by the night pack. **It is an owner ruling with a written spec — do not let it fall off the queue.**

## SHIPPED `v1.14.271` (full detail in the commit + `version.json`)
- **R1 — TWO CAMERAS**, held for the scene's life, switched **by reference**, both resized by `doResize`. **ISO = the JOHN-LOCKED `.255` perspective (FOV 28), still default + landing, and deliberately takes NO view-lock so its idle spin is unchanged.** **FRONT/REAR = orthographic** — a perspective camera *physically cannot* draw a true elevation (the splay is the projection, not the rack). **TOP ≈ 58° high-3/4.** Frustum from the rack itself (centred on y=0), 8% headroom; **verified numerically**: RH 16.32 spans ±8.16, halfH 8.81, halfW 28.67, distance 23.4–72.3 inside near/far. ⚠️ **`zoom` is a DISTANCE multiplier here, so ortho takes its RECIPROCAL** (1→1, 0.55→1.82, 1.7→0.59). Shortest-angular-path on theta.
- **`.264` STANDS**: drag still never writes `tElev`; `setView()` is the only writer. **NEW `_viewLock`**: a preset suppresses the idle auto-spin (a FRONT that slowly rotates is no longer an elevation); ISO sets no lock; any real orbit releases it.
- **R4 — EXPLODE**: rail **button**, never a gesture; **targets-only** (`userData.tExplodeZ`, lerped 0.07 in the ONE existing loop); the `(i%3-1)*0.3` **stagger IS the visual** (uniform explode = an uncountable slab); **translation only, no tilt**. **Sheet rule**: device sheet stows from inside the closure; phase sheet calls `window._reh3dStow`, **nulled in teardown** so it can never fire on a disposed context.

## ⭐ TWO SELF-CAUGHT HOISTING BUGS — **`node --check` CANNOT SEE EITHER. THIS IS A REPEATABLE TRAP IN THIS FILE.**
1. **`trayGroups`** was declared with the orbit state, **AFTER** the `slots.forEach` that pushes to it. `var` hoists the **binding** but not the **assignment** → `undefined.push` at runtime.
2. **`_railEl` / `_railBtns` / `_explodeBtn`** are **assigned** in the rail block but were re-declared **`var x = null` ~300 lines LATER**, **wiping the references after assignment** → `setView`'s is-on toggle silently no-ops **and teardown gets `null`, stranding the rail in the DOM forever.**
⭐ **RULE EARNED: in this 50k-line single-file app, `rackElevation_render3D` is ~1000 lines long and its declarations are scattered. ANY new `var` must be declared ABOVE its first assignment/use, and the check is mechanical: `decl-line < first-assign-line`. Run that audit on every new var — the gates are blind to it.** (Cousin of [[feedback_replace_all_self_reference]].)

## RULE 1 — the rail is its OWN row, deliberately
Appending `FRONT|ISO|TOP|REAR|EXPLODE` to `.reh-3d-toggle` = **SEVEN** buttons on one **no-wrap** flex row (~390px+) → pushes the viewport on a 390px phone. **That is the exact defect the GB300 reference file ships** (its 7-button `#controls` overflows both edges under `body{overflow:hidden}`, so FRONT and SCAN are unreachable). **PROVEN:** intrinsic 329px (one row at 390); forced to **320px and 280px** it **WRAPS to 2 rows and never overflows** — `flex-wrap` makes horizontal overflow structurally impossible.

## ⚠️ VERIFICATION BOUNDARY — READ BEFORE TRUSTING THIS SHIP
**Verified live:** rail renders (ISO on at boot) · stow hook idempotent · **teardown removes the rail + CABLES chip and NULLS the hook** · frustum math checked against real dims · **`?legacy=1` clean** (`render3D` 0 calls, no rail/strip, `_reh3dStow` undefined, `phdock_open` no-throw, host `#ops-content`).
⛔ **NOT verified — DEVICE ONLY:** *everything behind the frame loop.* **rAF NEVER fires in a hidden automation tab** (re-proven this session), so the loop never ran: the ortho **render**, the level FRONT/REAR read, TOP's framing, the explode **motion** + stagger, shortest-path animation, the idle-lock, **raycast/tap after the projection swap**, **shadow behaviour on extended trays**, 55fps. **The camera MATH is verified numerically; the PIXELS are the owner's eye.** This is exactly his Ship B checklist [6]–[15]. ⭐ **If Ship B ever needs pre-device proof, the answer is the ISOLATION HARNESS (`scratchpad/floorlab.html` pattern) driven by `setInterval`, NOT rAF.**

## DEVIATIONS (reported)
- **[a] No `tZoom` exists in this build** — the spec assumed presets write one. `zoom` is immediate, not lerped, so a preset **snaps zoom to 1** for a deterministic framing rather than adding new lerp machinery that would change pinch feel.
- **[b] "Cold Aisle sized" vs ".reh-3d-seg styling family" CONFLICT.** The family is **22px tall** (padding 4px / font 10px) — **below the 44px gloved floor**. Shipped the family for visual consistency with `FLAT|3D|CABLES` directly above. ⭐ **OPEN OWNER QUESTION: resize the whole seg family to 44px?** It is a one-line token change but it touches chrome he has already verified — **not a drive-by.**

## OWNER GATE (iPhone, gloves) — `.271` is the one unverified ship in flight
[6] FRONT **LEVEL**, no splay, U heights equal top→bottom · [7] REAR same · [8] **ISO identical to `.270`** · [9] TOP usable; drag during TOP still never tilts · [10] shortest-path, no long-way FRONT↔REAR spins · [11] EXPLODE out: stagger visible, internals readable; **home: EVERY tray returns fully** (the source file's close bug is the regression to hunt) · [12] explode → open sheet → stows; close sheet → **stays** stowed · [13] tap-to-inspect works in **both** projections and while exploded · [14] orbit/pinch unchanged in ISO; page scroll still owns vertical · [15] **watch tray shadows clipping the §A shadow frustum when extended (~+1.6 z)** — if so, a **minimal** shadow-camera widen next ship (technical accommodation, not aesthetic).
