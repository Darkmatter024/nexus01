...
**Last updated: 2026-09-01, after `v1.14.563`.
⏳ **ONE UNVERIFIED SHIP: `.563`.** `main` = **`.563`**; `release`, served and `VERIFIED` remain at **`.562`**, which was device-verified. `.563` is docs-visible only (dead-code removal, nothing renders differently) and awaits the owner's iPhone PASS and promotion. Through `.562`, every ship
...
| | |
|---|---|
| **Version** | **`phantom-v1.14.563`** — on `main` only. `release`, served and `VERIFIED` are at **`.562`**. ⏳ Gap 1: `release` promotion is Claude Code's per the 2026-08-30 ruling and follows this push; the **device verify and the `VERIFIED` stamp are owner-only and are what actually gate the ship**. `forge.html` carries NO version stamp and ships outside `version.json`'s scope. |
...
⭐ **SCAN IS RULED — v2 §2 WINS, THE BOARD LINE IS STALE (owner, 2026-09-01).** Three documents disagreed: `SHIP-HANDOFF-IA-SHIFTNAV-v2.md` §2 says SCAN is **not a dock destination** and lives where scanning happens — on the rack (scan a device into this rack) and on the picker (scan a rack label to open it); `PHANTOM-BOARD-NEXT-OPS-v2.md:40` says *"Scan gets a dock slot"*; `SHIP-HANDOFF-SCAN-PILLAR.md` is held for a five-pillar dock. **RULING: v2 §2 governs. The board line is STRUCK as stale** — it was written against the pre-v2 five-pillar model and is not the queue of record for nav. ⛔ **Do not build a SCAN dock slot.** The §3 requirement stands and is the real target: **scan reachable in ≤1 tap from wherever the tech is**, rack or picker — which a dock slot was only ever a means to. ⚠ **CONSEQUENCE FLAGGED, NOT RULED:** the SCAN-PILLAR handoff's central premise — SCAN becomes a nav pillar — is what v2 §2 just refused, so the document is no longer merely *deferred*; its purpose is contradicted. It was **not** named in this ruling and is left standing. What survives it regardless is evidence, not tasking: the ten routes at A-5 and the `pg-scan` history. **One more word from the owner retires it or re-scopes it; Claude Code does not retire a handoff by inference.** The superseded hold, kept for the record: ⛔ **SCAN-PILLAR WAS HELD UNTIL M4 — owner ruling 2026-09-01.** `SHIP-HANDOFF-SCAN-PILLAR.md` is in
