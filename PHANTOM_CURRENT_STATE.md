...
## 3 · Verify debt — ⏳ **ONE at `.583`.** ⛔ This heading read *"ZERO at `.557`"* for 24 versions; the debt below `.557` is the historical record

⏳ **OPEN: `.583`.** STATUS-HONESTY, on `main` (`aa4d437`), pushed, unpromoted, unstamped. **Owed,
in order:** `tools/promote.ps1` → the one device check (Build's strip reads `LOCAL ACTIVE | ON-DEVICE
· SAVED`, never SYNCED; offline still `ON-DEVICE · OFFLINE`) → `tools/verify.ps1 583 PASS` or
`tools/verify.ps1 583 FAIL "reason"` — one command that stamps, pushes `main`, and on PASS promotes and
prints `SERVED:` (landed 2026-09-08, see §9). Checklist `docs/BATCH1-583-EVIDENCE.md`.
...
⏳ **PARKED ON `.583` (`aa4d437`), PUSHED TO `main`, UNPROMOTED AND UNADJUDICATED (2026-09-08).**
**Verify debt is ONE and the ship gate is AT its limit** — no `.584` is possible, by hook
(`phantom-guard.js`) and by `promote.ps1` Guard 4, until John promotes `.583`, checks the strip on
the phone that serves it, and stamps it either way. ⛔ **BATCH-OODA IS NOT IN FORCE.** `OWNER-RULINGS.md` 2026-09-05 §"NOT YET IN
...
1. ⏳ **`.583`: `promote.ps1` → device → `verify.ps1 583 PASS` (or `FAIL "reason"`).** All John's. Checklist in `docs/BATCH1-583-EVIDENCE.md`. *(`.582` closed 2026-09-08: device PASS, stamped `9a9d6db`.)*
1b. **Then Batch 2, one version per phone pass, each on its own GO:** PCT-NA (the `Complete` cell reads `—` with no active deployment), then RACKS-NA (`cmd_masterRackCount` → `null`, hero + stat card read `—` with no Master) — `docs/DATA-HONESTY-COMMAND-PHASE0-EVIDENCE.md` §2–§3.
