...
**Last updated: 2026-09-03, after `v1.14.573`.
✅ **NO VERIFY DEBT AND NO GAP.** `main` = `release` = served = `VERIFIED` = **`.573`**, owner-verified on the iPhone 2026-09-03. ⛔ **AND THE ARC THAT GOT THERE IS THE PART TO READ: `.572` FAILED ON DEVICE AND IS STAMPED `phantom-v1.14.572 FAILED`.** It is the first recorded failure in this file, and the stamp form is new — the guard was taught that **VERIFIED records an adjudication, not only a pass** (`920f19b`), because the old gate compared VERIFIED to the old version and demanded a match, which assumed every ship eventually passes. **A failed version can never honestly be stamped as passed, so `.573` — which existed only to fix `.572` — was blocked behind it forever.** A gate that forbids shipping the fix for the defect it just caught is inverted. ⭐ **`FAILED` is NOT a pass and licenses no promotion**; grep proved the guard is the only reader of `VERIFIED`, and an agent still cannot stamp either outcome.
