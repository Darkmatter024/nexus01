...
**Last updated: 2026-08-30, after `v1.14.533` — LEGACY-RETIRE Stage 1 shipped to `main`.
⏳ ONE UNVERIFIED SHIP: `.533`. It is on `main` and NOT promoted, so the phone still holds `.532`.
`VERIFIED` = `.532`, which is what the gate needs to accept `.533`; the NEXT bump needs `.533`.
...
| | |
|---|---|
| **Version** | **`phantom-v1.14.533`** (on `main`, unpromoted) / **`.532`** (baseline, and what `release` still serves) |
...
| Next Ship | ⛔ **BLOCKED on John: promote `.533`, verify, stamp.** Then **LEGACY-RETIRE Stage 2** — the census's 3 RETIRED rows, **re-verified against `.533` first**: `PHASE0-CENSUS.md` is based on `v1.14.166` and is 367 versions stale, so its verdicts are a map, not truth. 1–2 ships. |
