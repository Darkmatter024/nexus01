...
**Last updated: 2026-09-02, after `v1.14.569`.
✅ **NO VERIFY DEBT AND NO GAP.** `main` = `release` = served = `VERIFIED` = **`.569`**. `.563` through `.569` were each device-verified by the owner as they landed, **one at a time — seven ships, seven passes, never stacked.** Every ship
...
| | |
|---|---|
| **Version** | **`phantom-v1.14.569`** — on `main`, on `release`, served, and stamped in `VERIFIED`. **Gap 0.** ⭐ `release` promotion is Claude Code's per the 2026-08-30 ruling; the **device verify and the `VERIFIED` stamp are owner-only and are what actually gate a ship**. `forge.html` carries NO version stamp and ships outside `version.json`'s scope. |
