...
## 3 · Verify debt — ⏳ **ONE at `.581`.** ⛔ This heading read *"ZERO at `.557`"* for 24 versions; the debt below `.557` is the historical record

⏳ **OPEN: `.581`.** On `main` (`5cfabbc`), unpromoted, unstamped. **The one thing owed is John's
device verify against `main`'s URL**, then `tools/stamp.ps1 VERIFIED` and `tools/promote.ps1`, both
from his own terminal. The private-tab checklist is in `docs/BATCH1-581-EVIDENCE.md`; the one check
that matters is that the **RACKS stat cell and the hero RACKS KPI show the same number, and that it
is the Master's cab count** — not the deployment's rack count.
...
⏳ **PARKED ON `.581`, PUSHED TO `main` AND UNADJUDICATED. Nothing autonomous, nothing in flight.**
**Verify debt is ONE and the ship gate is AT its limit** — no `.582` is possible, by rule and by
hook, until John verifies `.581` on `main`'s URL and stamps it.

**The queue behind that gate, in order:**
