...
| Next Ship | ⛔ **BLOCKED on John: promote `.535`, verify, stamp.** ⚠ **This verify is different from the last two** — `.535` removed LIVE legacy UI, so the check is under `?legacy=1`, not on the default app: the NOW dashboard should have lost its TODAY card and kept everything else. Then **Stage 3 (LIVE shells)** — legacy shells of the 18 LIVE surfaces, 4–6 ships, medium risk, approved contingent on 0–2 landing clean. |
