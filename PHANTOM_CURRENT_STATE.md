...
| | |
|---|---|
| **Version** | **`phantom-v1.14.563`** — on `main` only. `release`, served and `VERIFIED` are at **`.562`**. ⏳ Gap 1 by design: the owner promotes and stamps, and `.563` has neither yet. `forge.html` carries NO version stamp and ships outside `version.json`'s scope. |
