...
**Last updated: 2026-08-30, after `v1.14.534` — LEGACY-RETIRE Stage 2a shipped to `main`.
⏳ ONE UNVERIFIED SHIP: `.534`, on `main`, NOT promoted. `VERIFIED` = `.533`, device-verified
2026-08-30. `.525`–`.533` are all verified and stamped; there is no older debt behind this one.
Next: John promotes `.534`, verifies, stamps — then Stage 2b (TODAY Pulse).**
...
| | |
|---|---|
| **Version** | **`phantom-v1.14.534`** (on `main`, unpromoted) / **`.533`** (baseline — `release` and the served bytes, device-verified) |
...
| Next Ship | ⛔ **BLOCKED on John: promote `.534`, verify, stamp.** Then **Stage 2b — TODAY Pulse**, the last of the three RETIRED rows. ⚠ **Not a deletion of dead code:** the pulse section (`// ── PULSE SECTION ──` … `// end pulse card`, ~29 lines) sits inside `today_render`, which has **four live callers including one at boot**, and it **renders under `?legacy=1`** as the legacy NOW dashboard's pulse card. So 2b is a **visible legacy change** and surgery inside live code — the opposite of 2a on both counts. See the re-verification block in `PHASE0-CENSUS.md`. |
